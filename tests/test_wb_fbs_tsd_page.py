...
def test_tsd_js_uses_dedicated_api_prefix() -> None:
    ...
    assert "КИЗ записан · ${label}" in js
    ...
    assert "rowNeedsKizWbPush" in js
    assert "skip unchanged already-synced codes" in js
    assert "applyKizPushResults" in js
    # Local autosave must clear WB-synced flag (else floppy/leave skip push).
    flush = js[
        js.find("async function flushKizLocalAutosave") : js.find(
            "async function flushPickLocalAutosave"
        )
    ]
    assert "kiz_wb_synced = false" in flush
    assert 'refreshScanChrome("kiz")' in flush
    assert "scheduleKizWbAutoPush(id)" in flush
    # Scan placement also marks unsynced before background autosave finishes.
    assert "Mark unsynced immediately so floppy" in js
    assert "function scheduleKizWbAutoPush" in js
    assert "async function flushKizWbAutoPush" in js
    assert "wbAutoPushChain" in js
    assert "Fire-and-forget WB push" in js
...
def test_tsd_phone_camera_scan_button() -> None:
    ...
    assert "wb_fbs_tsd.js?v=93" in html
    ...
    assert "Card rebuild replaces #tsdScanInput" in js
    assert "never focus the old (detached) node" in js
    # After success, focus live input — not the node replaced by outerHTML.
