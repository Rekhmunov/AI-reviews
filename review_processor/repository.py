...
class ReviewRepository:
    ...
    def add_product_photo(
        ...
        with self._connect() as conn:
            product_id = self._insert_and_get_id(
                conn,
                self._sql("""
                INSERT INTO product_photos (user_id, name, supplier_article, wb_nmid, ozon_sku, photo_path, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """),
                (user_id, name.strip(), supplier_article.strip(), wb_nmid.strip(), ozon_sku.strip(), photo_path, now, now),
            )
    ...
    def update_product_photo(
        ...
        now = _utc_now()
        sets = ["name=?", "supplier_article=?", "wb_nmid=?", "ozon_sku=?", "updated_at=?"]
        params: list[Any] = [name.strip(), supplier_article.strip(), wb_nmid.strip(), ozon_sku.strip(), now]
    ...
    def list_supply_legal_entities(self, *, user_id: int) -> list[dict[str, Any]]:
        with self._connect() as conn:
            rows = conn.execute(
                self._sql("SELECT * FROM supply_legal_entities WHERE user_id = ? ORDER BY short_name ASC"),
                (user_id,),
            ).fetchall()
        return [self._row_to_dict(row) for row in rows]

    def update_supply_legal_entity(self, *, user_id: int, entity_id: int, short_name: str, full_name: str, requisites: str = "", signatories: str = "", in_person: str = "", basis: str = "") -> bool:
        with self._connect() as conn:
            result = conn.execute(
                self._sql("UPDATE supply_legal_entities SET short_name = ?, full_name = ?, requisites = ?, signatories = ?, in_person = ?, basis = ? WHERE user_id = ? AND id = ?"),
                (short_name.strip(), full_name.strip(), (requisites or "").strip() or None, (signatories or "").strip() or None, (in_person or "").strip() or None, (basis or "").strip() or None, user_id, entity_id),
            )
        return bool(result.rowcount)

    def create_supply_legal_entity(self, *, user_id: int, short_name: str, full_name: str, requisites: str = "", signatories: str = "", in_person: str = "", basis: str = "") -> dict[str, Any]:
        now = _utc_now()
        with self._connect() as conn:
            eid = self._insert_and_get_id(
                conn,
                "INSERT INTO supply_legal_entities (user_id, short_name, full_name, requisites, signatories, in_person, basis, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (user_id, short_name.strip(), full_name.strip(), (requisites or "").strip() or None, (signatories or "").strip() or None, (in_person or "").strip() or None, (basis or "").strip() or None, now),
            )
