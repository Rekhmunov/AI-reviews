...
class ReviewRepository:
    ...
    def create_certificate(self, *, user_id: int, legal_entity_short: str, category: str,
                           ...
                           image_data: str | None) -> int:
        now = _utc_now()
        with self._connect() as conn:
            row = conn.execute(
                self._sql(
                    "INSERT INTO supply_certificates "
                    "(user_id, legal_entity_short, category, number, expiry_date, verification_url, image_data, created_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id"
                ),
                (user_id, legal_entity_short.strip(), category.strip(), number.strip(),
                 expiry_date.strip(), verification_url.strip(), image_data, now),
            ).fetchone()
        return int(self._row_to_dict(row)["id"]) if row else 0
    ...
    def _ensure_salary_workers_table(self, conn) -> None:
        ...
        """)
        # Idempotent additions for existing tables
    ...
    def list_salary_workers(self, *, owner_user_id: int) -> list[dict[str, Any]]:
        with self._connect() as conn:
            ...
            rows = conn.execute(
                self._sql(
                    "SELECT id, owner_user_id, full_name, birth_date, legal_entity, production, created_at "
    ...
    def create_salary_worker(
        self, *, owner_user_id: int, full_name: str, birth_date: str,
        legal_entity: str, production: str,
    ) -> dict[str, Any]:
        now = _utc_now()
        with self._connect() as conn:
            self._ensure_salary_workers_table(conn)
            cur = conn.execute(
                self._sql(
                    ...
                    "(owner_user_id, full_name, birth_date, legal_entity, production, created_at) "
                    "VALUES (?, ?, ?, ?, ?, ?) RETURNING id"
                ),
                (owner_user_id, full_name.strip(), birth_date.strip(),
        ...
        return {"id": worker_id, "owner_user_id": owner_user_id,
                "full_name": full_name.strip(), "birth_date": birth_date.strip(),
                "legal_entity": legal_entity.strip(), "production": production.strip(),
                "created_at": now}
