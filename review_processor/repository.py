...
class ReviewRepository:
    ...
    def list_salary_workers(self, *, owner_user_id: int) -> list[dict[str, Any]]:
        with self._connect() as conn:
            ...
            rows = conn.execute(
                self._sql(
                    "SELECT id, owner_user_id, full_name, production, created_at "
    ...
    def create_salary_worker(
        self, *, owner_user_id: int, full_name: str, production: str
    ) -> dict[str, Any]:
        now = _utc_now()
        with self._connect() as conn:
            self._ensure_salary_workers_table(conn)
            cur = conn.execute(
                self._sql(
                    "INSERT INTO salary_workers (owner_user_id, full_name, production, created_at) "
                    "VALUES (?, ?, ?, ?) RETURNING id"
                ),
                (owner_user_id, full_name.strip(), production.strip(), now),
            )
            row = cur.fetchone()
        worker_id = int(row[0] if not hasattr(row, "get") else row.get("id"))
        return {"id": worker_id, "owner_user_id": owner_user_id,
                "full_name": full_name.strip(), "production": production.strip(),
                "created_at": now}
