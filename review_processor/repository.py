...
class ReviewRepository:
    ...
    def update_supply_legal_entity(self, *, user_id: int, entity_id: int, short_name: str, full_name: str, requisites: str = "", signatories: str = "", in_person: str = "") -> bool:
        with self._connect() as conn:
            result = conn.execute(
                self._sql("UPDATE supply_legal_entities SET short_name = ?, full_name = ?, requisites = ?, signatories = ?, in_person = ? WHERE user_id = ? AND id = ?"),
                (short_name.strip(), full_name.strip(), (requisites or "").strip() or None, (signatories or "").strip() or None, (in_person or "").strip() or None, user_id, entity_id),
            )
        return bool(result.rowcount)

    def create_supply_legal_entity(self, *, user_id: int, short_name: str, full_name: str, requisites: str = "", signatories: str = "", in_person: str = "") -> dict[str, Any]:
        now = _utc_now()
        with self._connect() as conn:
            eid = self._insert_and_get_id(
                conn,
                "INSERT INTO supply_legal_entities (user_id, short_name, full_name, requisites, signatories, in_person, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (user_id, short_name.strip(), full_name.strip(), (requisites or "").strip() or None, (signatories or "").strip() or None, (in_person or "").strip() or None, now),
            )
