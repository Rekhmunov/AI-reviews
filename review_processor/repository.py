...
class ReviewRepository:
    ...
    def update_supply_manual_fields(
        ...
        *,
        ...
        with self._connect() as conn:
            # Update display columns in supply_items
            result = conn.execute(
                self._sql(
                    """
                    UPDATE supply_items
                ...
                (pn, pc, dn, supply_id, user_id),
            )
            # Persist to permanent store (survives clear_supply_items)
            conn.execute(
                self._sql(
                    """
                    INSERT INTO supply_manual_data
                        (user_id, supply_id, pass_number, pallets_count, driver_name, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                ...
                (user_id, supply_id, pn, pc, dn, now),
            )
        return bool(result.rowcount)
