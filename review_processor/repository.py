...
class ReviewRepository:
    ...
    def _migrate_supply_tables(self, conn) -> None:
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS supply_ttn_records (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id BIGINT NOT NULL,
                doc_number TEXT NOT NULL DEFAULT '',
                ttn_date TEXT NOT NULL,
                legal_entity_id BIGINT NOT NULL,
                contractor_id BIGINT NOT NULL,
                driver_id BIGINT NOT NULL DEFAULT 0,
                driver_manual_name TEXT,
                driver_manual_docs TEXT,
                vehicle_line TEXT NOT NULL DEFAULT '',
                carrier_snapshot TEXT NOT NULL DEFAULT '',
                load_address TEXT NOT NULL DEFAULT '',
                unload_address TEXT NOT NULL DEFAULT '',
                cargo_description TEXT NOT NULL DEFAULT '',
                cargo_places TEXT NOT NULL DEFAULT '',
                cargo_weight TEXT NOT NULL DEFAULT '',
                accompanying_docs TEXT NOT NULL DEFAULT '',
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS ttn_edo_documents (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id BIGINT NOT NULL,
                ttn_record_id BIGINT NOT NULL,
                doc_type TEXT NOT NULL,
                channel TEXT NOT NULL DEFAULT '',
                transportation_id TEXT NOT NULL DEFAULT '',
                message_id TEXT NOT NULL DEFAULT '',
                ...
                sent_at TEXT NOT NULL DEFAULT '',
                updated_at TEXT NOT NULL DEFAULT '',
                UNIQUE (user_id, ttn_record_id, doc_type)
            )
            """
        )
        conn.execute(
            self._sql(
                "CREATE INDEX IF NOT EXISTS idx_ttn_edo_docs_user_ttn "
                "ON ttn_edo_documents(user_id, ttn_record_id)"
            )
        )
        conn.execute(
            """
            ...
            rows = conn.execute(
                self._sql("""
                    SELECT t.id, t.user_id, t.doc_number, t.ttn_date, t.created_at,
                           ...
                           d.carrier_kpp AS d_carrier_kpp, d.carrier_phone AS d_carrier_phone,
                           d.doc_vu_series AS d_vu_series, d.doc_vu_number AS d_vu_number,
                           d.doc_vu_issuer AS d_vu_issuer, d.doc_vu_date AS d_vu_date,
                           d.doc_inn_fl AS d_inn_fl,
                           t.driver_manual_name, t.driver_manual_docs,
        ...
        *,
        ...
        with self._connect() as conn:
            rid = self._insert_and_get_id(
                ...
                "user_id, doc_number, ttn_date, title, legal_entity_id, contractor_id, "
                ...
                (
                    ...
                    (vehicle_line or "").strip(),
                    (carrier_snapshot or "").strip(),
                    (load_address or "").strip(),
                    (unload_address or "").strip(),
        ...
        with self._connect() as conn:
            result = conn.execute(
                self._sql(
                    ...
                    "ttn_date = ?, title = ?, legal_entity_id = ?, contractor_id = ?, "
                    "shipper_type = ?, consignee_type = ?, driver_id = ?, "
    ...
    def delete_supply_ttn_record(self, *, user_id: int, record_id: int) -> bool:
        with self._connect() as conn:
            result = conn.execute(
                self._sql("DELETE FROM supply_ttn_records WHERE user_id = ? AND id = ?"),
                (user_id, record_id),
            )
        return bool(result.rowcount)
    ...
    def list_ttn_edo_documents(self, *, user_id: int, ttn_record_id: int) -> list[dict[str, Any]]:
        with self._connect() as conn:
            self._migrate_supply_tables(conn)
            rows = conn.execute(
                self._sql(
                    "SELECT * FROM ttn_edo_documents WHERE user_id = ? AND ttn_record_id = ? "
                    "ORDER BY doc_type ASC"
                ),
                (user_id, ttn_record_id),
            ).fetchall()
        return [self._row_to_dict(r) for r in rows]

    def upsert_ttn_edo_document(
        self,
        *,
        user_id: int,
        ttn_record_id: int,
        ...
        with self._connect() as conn:
            ...
            existing = conn.execute(
                self._sql(
                    "SELECT * FROM ttn_edo_documents "
                    "WHERE user_id = ? AND ttn_record_id = ? AND doc_type = ?"
                ),
                (user_id, ttn_record_id, doc_type),
            ).fetchone()
            if existing:
                prev = self._row_to_dict(existing)
                sent_at = str(prev.get("sent_at") or "")
                ...
                conn.execute(
                    self._sql(
                        "UPDATE ttn_edo_documents SET channel = ?, transportation_id = ?, message_id = ?, "
                        "entity_id = ?, status = ?, status_label = ?, mintrans_id = ?, mintrans_status = ?, "
                        "last_error = ?, raw_json = ?, sent_at = ?, updated_at = ? "
                        "WHERE user_id = ? AND ttn_record_id = ? AND doc_type = ?"
                    ),
                ...
                conn.execute(
                    self._sql(
                        "INSERT INTO ttn_edo_documents ("
                        "user_id, ttn_record_id, doc_type, channel, transportation_id, message_id, entity_id, "
                        "status, status_label, mintrans_id, mintrans_status, last_error, raw_json, sent_at, updated_at"
                        ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                    ),
                    (
    ...
    def _ensure_supply_balances_tables(self, conn) -> None:
        ...
        # Ozon FBS postings frozen after stock opening/adjustment (by posting_number).
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS supply_stock_ozon_fbs_settled (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                user_id BIGINT NOT NULL,
        ...
        conn.execute(
            self._sql(
                "CREATE INDEX IF NOT EXISTS idx_supply_stock_ozon_fbs_settled_user "
                "ON supply_stock_ozon_fbs_settled(user_id)"
            )
        )
    ...
    def get_product_id_by_ozon_keys_map(self, *, user_id: int) -> dict[str, int]:
        """Map supplier_article / ozon_sku (+ casefold) → product_photos.id for Ozon FBS."""
        rows = self.list_product_photos(user_id=user_id)
        result: dict[str, int] = {}
        for r in rows:
            try:
    ...
    def _ozon_fbs_stock_net_shipped(
        self, conn, *, user_id: int, posting_number: str, product_id: int
    ) -> tuple[float, int, int]:
        """Return (net_shipped_units, ship_rows, reverse_rows) for one posting line."""
        ...
        rows = conn.execute(
            self._sql(
                """
                SELECT kind, source_type, qty FROM supply_stock_movements
                WHERE user_id = ?
                  AND (
                    (source_type = 'ozon_fbs_posting' AND source_id LIKE ?)
                    OR
                    (source_type = 'ozon_fbs_posting_reverse' AND source_id LIKE ?)
                  )
                """
            ),
        ...
        for r in rows:
            ...
            try:
                ...
                qty = 0.0
            if src_type == "ozon_fbs_posting_reverse" or kind == "fbs_reverse":
                reverses += 1
                net -= abs(qty)
            elif src_type == "ozon_fbs_posting" or kind == "fbs_ship":
                ships += 1
                net += abs(qty)
        return net, ships, reverses

    def is_ozon_fbs_posting_stock_settled(
        self, conn, *, user_id: int, posting_number: str
    ) -> bool:
        pn = str(posting_number or "").strip()
        if not pn:
            return False
        row = conn.execute(
            self._sql(
                "SELECT 1 AS ok FROM supply_stock_ozon_fbs_settled "
                "WHERE user_id = ? AND posting_number = ? LIMIT 1"
            ),
            (user_id, pn),
        ).fetchone()
        return bool(row)

    def settle_open_ozon_fbs_postings_for_stock(
        self,
        *,
        user_id: int,
        production_id: int,
    ...
    def reconcile_ozon_fbs_stock_postings(
        ...
        *,
        ...
        """Align ledger with Ozon FBS tabs (idempotent).

        Desired state (Поставки → Остатки):
        - ``delivering`` / ``delivered`` → net shipped == line qty (left warehouse)
        - ``awaiting_packaging`` / ``awaiting_deliver`` → net shipped == 0
        - ``cancelled`` / ``arbitration`` / unknown → leave as-is
        - rows in ``supply_stock_ozon_fbs_settled`` are ignored

        Does not touch WB FBS ledger rows.
        """
        stats = {"shipped": 0, "reversed": 0, "skipped": 0, "ok": 0, "settled": 0}
        if not postings or int(production_id or 0) <= 0:
            return stats
        date_s = str(movement_date or "").strip()
        if not date_s:
            return stats
        id_map = self.get_product_id_by_ozon_keys_map(user_id=user_id)
        now = _utc_now()

        def _line_products(row: dict[str, Any]) -> list[tuple[str, str, float]]:
            """Return list of (offer_id, sku, qty)."""
            out: list[tuple[str, str, float]] = []
            raw_products = row.get("products_json")
            parsed: list[Any] = []
            if isinstance(raw_products, list):
                parsed = raw_products
            elif isinstance(raw_products, str) and raw_products.strip():
                try:
                    loaded = json.loads(raw_products)
        ...
        with self._connect() as conn:
            ...
            for row in postings:
                ...
                if not pn:
                    stats["skipped"] += 1
                    continue
                if self.is_ozon_fbs_posting_stock_settled(
                    conn, user_id=user_id, posting_number=pn
                ):
                    stats["settled"] += 1
                    continue
                tab = str(row.get("tab") or "").strip().lower()
                lines = _line_products(row)
                if not lines:
                    stats["skipped"] += 1
                    continue

                for offer, sku, line_qty in lines:
                    product_id = 0
                    for key in (offer, offer.casefold(), sku, sku.casefold()):
                        if key and key in id_map:
                            product_id = int(id_map[key])
                            break
                    if product_id <= 0:
                    ...
                    net, ships, reverses = self._ozon_fbs_stock_net_shipped(
                        conn,
                        user_id=user_id,
                        posting_number=pn,
                    ...
                    def _insert(
                        kind: str, qty: float, source_type: str, source_id: str
                    ) -> bool:
                        cur = conn.execute(
                            self._sql(
                                "INSERT INTO supply_stock_movements "
                    ...
                    if tab in {"delivering", "delivered"}:
                        if net + 1e-9 >= line_qty:
                            stats["ok"] += 1
                            continue
                        need = line_qty - net
                        seq = ships + 1
                        source_id = f"{pn}:p{product_id}:s:{seq}"
                        if _insert("fbs_ship", -float(need), "ozon_fbs_posting", source_id):
                            stats["shipped"] += 1
                        else:
                            stats["skipped"] += 1
                    elif tab in {"awaiting_packaging", "awaiting_deliver"}:
                        if net <= 1e-9:
                            stats["ok"] += 1
                            continue
                        seq = reverses + 1
                        source_id = f"{pn}:p{product_id}:r:{seq}"
                        if _insert(
                            "fbs_reverse",
                            float(net),
