...
def build_ttn_prefill(
    ...
    *,
    ...
    """Local draft for «Сформировать ТН» from an Ozon FBS supply."""
    ...
    with repo._connect() as conn:
        row = conn.execute(
            repo._sql(
                """
                SELECT name, created_at, warehouse_name
                FROM ozon_fbs_supplies
                WHERE user_id = ? AND source_id = ? AND supply_id = ?
                """
            ),
    ...
    if local:
        ...
        created_at = str(local.get("created_at") or "").strip()

    shipper = repo.find_legal_entity_for_fbs_source(
        user_id=user_id, platform="ozon", source_id=src
    )
    ...
    existing_id = repo.find_ttn_record_id_by_fbs(
        user_id=user_id, platform="ozon", source_id=src, supply_id=sid
    )
    ...
    if driver_id > 0:
        ...
        if drow:
            ...
            if match:
                vehicle_line = str(
                    match.get("line") or match.get("number") or vehicle_number
                ).strip()
    ...
    with repo._connect() as conn:
        orows = conn.execute(
            repo._sql(
                """
                SELECT posting_number, offer_id, sku, quantity, status, tab
                FROM ozon_fbs_postings
                WHERE user_id = ? AND source_id = ? AND supply_id = ?
                ORDER BY posting_number ASC
                """
    ...
    for orow in orows:
        ...
        if oz.posting_row_is_cancelled(od):
            od["cancel_reason_label"] = "cancelled"
            od["cancelled"] = True
        orders.append(od)
...
def list_supplies_for_driver_vehicle(
    ...
    *,
    ...
    for row in rows:
        ...
        created_at = str((supply or {}).get("created_at") or "").strip()
        out.append(
            {
                "source_id": source_id,
                "supply_id": supply_id,
    ...
    *,
            ...
            items.append(
                {
                    ...
                    "supply_created_at": str(
                        supply.get("supply_created_at") or ""
                    ).strip(),
                    "warehouse_name": str(
    ...
    def _supply_created_ms(value: object) -> float:
        ...
        if not raw:
            return 0.0
        try:
            # Accept ISO / space-separated timestamps.
    ...
    items.sort(
        key=lambda r: (
            # Nearest supply date first (missing dates last).
