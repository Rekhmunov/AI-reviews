...
(function () {
  ...
  function showCollectResult(data) {
    ...
    let html = `<p class="${ok ? "ozon-fbs-collect-result-ok" : "ozon-fbs-collect-result-err"}">${esc(data?.message || "")}</p>`;
    if (groupLines.length) {
      html += "<ul>" + groupLines.map((g) => `<li>${esc(g)}</li>`).join("") + "</ul>";
    }
    if (created.length) {
      html += `<p class="ozon-fbs-collect-result-section-title">Созданы поставки:</p><ul>` +
        created.map((s) => `<li>${esc(s.name || s.supply_id || "")}</li>`).join("") + "</ul>";
    }
    if (errors.length) {
      html += `<p class="ozon-fbs-collect-result-err ozon-fbs-collect-result-section-title">Ошибки:</p>` +
        `<ul class="ozon-fbs-collect-err-list">` +
        errors.map((e) => formatCollectErrorItemHtml(e)).join("") + "</ul>";
    }
    body.innerHTML = html;
    body.classList.add("ozon-fbs-collect-result-body");
    modal.classList.remove("hidden");
  }
  ...
  function renderCollectModal(preview) {
    ...
    body.innerHTML = groups.map((g, idx) => {
      ...
      if (mode === "create") {
        ...
        inner = `
          <div class="ozon-fbs-collect-field">
            ...
            <p class="ozon-fbs-collect-warn" id="ozonFbsCollectWarn_${esc(gkey)}" ${conflict ? "" : "hidden"}>
        ...
        inner = `
          <div class="ozon-fbs-collect-field">
            <div class="ozon-fbs-collect-field-label">Куда собрать заказы</div>
            <div class="ozon-fbs-collect-supplies" role="radiogroup" aria-label="Куда собрать заказы">
              ${supplies.map((s, si) => {
          ...
          <div class="ozon-fbs-collect-field ozon-fbs-collect-new-name" id="ozonFbsCollectNewWrap_${esc(gkey)}"
  ...
  function toggleOzonFbsRowMenu(event, menuKey) {
    ...
    if (!menu || !btn) return;
    _ozonFbsSyncRowMenuContent(menu);
  ...
  function _ozonFbsRowIsCancelled(row) {
    ...
    // Align with backend posting_row_is_cancelled (status/tab).
    const tab = String(row.tab || "").trim().toLowerCase();
    const status = String(row.status || "").trim().toLowerCase();
    if (tab === "cancelled") return true;
    if (status === "cancelled" || status === "cancelled_from_split_pending") return true;
    return false;
  }
  ...
  function renderSupplyDetail(data) {
    ...
    tbody.innerHTML = orders.map((o) => {
      ...
      const removeCancelledSd = _ozonFbsRowIsCancelled(o) || cancelLabel
        ? `<button type="button" class="wb-fbs-row-menu-item" role="menuitem"
                  onclick="ozonFbsRemoveCancelledPostingFromSupply(event, '${esc(pn)}')">
      ...
      const actCell = readOnly
        ...
        : `<td class="wb-fbs-sd-col-act">
          <div class="wb-fbs-row-menu-wrap" id="ozonFbsRowMenuWrap_${sdMenuKey}">
            <button type="button" class="icon-btn secondary wb-fbs-row-menu-btn" title="Действия"
                    onclick="toggleOzonFbsRowMenu(event, '${sdMenuKey}')" aria-haspopup="menu">⋮</button>
            <div id="ozonFbsRowMenu_${sdMenuKey}" class="wb-fbs-row-menu" data-posting="${esc(pn)}" data-mode="sd" role="menu">
              <button type="button" class="wb-fbs-row-menu-item" role="menuitem"
                      data-ozon-action="print-sticker" data-posting="${esc(pn)}">
                Напечатать стикер
              </button>
  ...
  /** In-flight silent status checks (GM cleared by Ozon) — do not block scan UI. */
  const ozonFbsSilentStatusInflight = new Set();

  /**
   * Apply cancel flag without stealing scan focus: bump «Отменённые», patch row.
   * No toasts / status modal.
   */
  function _ozonFbsApplyCancelledQuiet(pn, cancelLabel) {
    const posting = String(pn || "").trim();
    if (!posting) return;
    const label = String(cancelLabel || "Отменено").trim() || "Отменено";
    const active = document.activeElement;
    const activeId = active && active.id ? String(active.id) : "";
    const activePn = active && active.dataset ? String(active.dataset.posting || "") : "";
    ...
    for (const row of [...(ozonFbsKizState.rows || []), ...(ozonFbsPickState.rows || [])]) {
      if (String(row?.posting_number || "").trim() !== posting) continue;
      row.cancelled = true;
      row.cancel_reason_label = label;
      row.tab = "cancelled";
      row.status = "cancelled";
      // GM drop on cancel is not an assembly error.
      row.container_sync_error = "";
    }

    // Prefer light DOM patch; fall back to full refresh with focus restore.
    const patched = _ozonFbsPatchCancelledRowDom(posting);
    if (!patched) {
      _ozonFbsRefreshOpenModalsAfterCancelFlag();
    } else {
      if (typeof _ozonFbsKizUpdateFilterCounts === "function") _ozonFbsKizUpdateFilterCounts();
      if (typeof _ozonFbsPickUpdateFilterCounts === "function") _ozonFbsPickUpdateFilterCounts();
      if (typeof _ozonFbsKizUpdateScanCounter === "function") _ozonFbsKizUpdateScanCounter();
      if (typeof _ozonFbsPickUpdateScanCounter === "function") _ozonFbsPickUpdateScanCounter();
      if (typeof window._ozonFbsContainerUpdateCounters === "function") {
        try { window._ozonFbsContainerUpdateCounters(); } catch (_e) { /* ignore */ }
      }
      _ozonFbsKizSplitSetTone(_ozonFbsKizToneFromSupply(supplyDetailState.supply));
      _ozonFbsPickSplitSetTone(_ozonFbsPickToneFromSupply(supplyDetailState.supply));
      _ozonFbsSupplyDetailUpdateNewWarn();
    ...
    requestAnimationFrame(() => {
      let el = activeId ? document.getElementById(activeId) : null;
      if (!el && activePn) {
        el = document.querySelector(
          `#ozonFbsKizTbody [data-posting="${CSS.escape(activePn)}"].wb-fbs-kiz-code-input,` +
          `#ozonFbsPickTbody [data-posting="${CSS.escape(activePn)}"] input,` +
          `#ozonFbsKizStickerScan, #ozonFbsPickStickerScan`
        );
      }
      if (!el) {
        el = document.getElementById("ozonFbsKizStickerScan")
          || document.getElementById("ozonFbsPickStickerScan");
      }
  ...
  function _ozonFbsPatchCancelledRowDom(pn) {
    const posting = String(pn || "").trim();
    if (!posting) return false;
    const rows = document.querySelectorAll(
      `#ozonFbsKizTbody tr.wb-fbs-kiz-row[data-posting="${CSS.escape(posting)}"],` +
      `#ozonFbsPickTbody tr.wb-fbs-kiz-row[data-posting="${CSS.escape(posting)}"]`
    );
    ...
    rows.forEach((tr) => {
      ...
      tr.querySelectorAll("input.wb-fbs-kiz-code-input, input.wb-fbs-pick-code-input").forEach((inp) => {
        inp.readOnly = true;
        inp.tabIndex = -1;
      });
      tr.querySelectorAll("button.wb-fbs-kiz-remove, button.wb-fbs-pick-remove").forEach((btn) => {
        btn.disabled = true;
      });
      tr.querySelectorAll("button.wb-fbs-kiz-add").forEach((btn) => {
        btn.remove();
      });
      const menu = tr.querySelector(".wb-fbs-row-menu[data-mode='kiz'], .wb-fbs-row-menu[data-mode='pick']");
      if (menu && typeof _ozonFbsSyncRowMenuContent === "function") {
        _ozonFbsSyncRowMenuContent(menu);
      }
    });
    return any;
  ...
  async function refreshOzonFbsModalPostingStatus(postingNumber, opts) {
    const silent = !!(opts && opts.silent);
    const pn = String(postingNumber || "").trim();
    if (!pn) return;
    if (!silent && ozonFbsPostingStatusState.busy) return;
    if (silent) {
      if (ozonFbsSilentStatusInflight.has(pn)) return;
      const existing = [...(ozonFbsKizState.rows || []), ...(ozonFbsPickState.rows || [])]
        .find((r) => String(r?.posting_number || "").trim() === pn);
      if (existing && _ozonFbsRowIsCancelled(existing)) return;
      ozonFbsSilentStatusInflight.add(pn);
    }
    const sourceId = supplyDetailState.sourceId || state.sourceId;
    if (!sourceId) {
      if (!silent) {
      ...
      if (silent) ozonFbsSilentStatusInflight.delete(pn);
      return;
    }
    if (!silent) {
    ...
    try {
      ...
      if (cancelled) {
        if (silent) {
          _ozonFbsApplyCancelledQuiet(pn, cancelLabel || "Отменено");
        } else {
          _ozonFbsCancelledMergeIntoDetail([
            {
              posting_number: pn,
              ...
              tab: tab || item.tab || "",
              cancel_reason_label: cancelLabel || "Отменено",
              cancelled,
            },
          ]);
          for (const row of [...(ozonFbsKizState.rows || []), ...(ozonFbsPickState.rows || [])]) {
          ...
          _ozonFbsRefreshOpenModalsAfterCancelFlag();
          // Short ack only — no long instructions (they distract while scanning).
          _ozonFbsPostingStatusRender({
            title: "Заказ отменён",
            html:
              `<p class="ozon-fbs-move-delivering-text">` +
              `Отправление <strong>${esc(pn)}</strong> — отменено` +
      ...
      // Silent failures stay silent — do not interrupt scanning.
    } finally {
      if (silent) {
        ozonFbsSilentStatusInflight.delete(pn);
      } else {
...
<p>Можно закрыть эту вкладку и повторить на странице поставки.</p>
  ...
  function _ozonFbsSyncOwnerOnlyAllCancellationsBtn() {
    const btn = document.getElementById("ozonFbsAllCancellationsBtn");
    if (!btn) return;
    const can = typeof isTenantOwner === "function" && isTenantOwner();
    btn.hidden = !can;
    btn.style.display = can ? "" : "none";
  ...
  function _ozonFbsContainersListParams(sourceId) {
  ...
  function _ozonFbsContainersSyncBusyUi() {
    ...
    _ozonFbsContainersSyncShowScUi();
  }
  ...
  function _ozonFbsContainersDetailHtml(cid) {
    ...
    if (!count) {
      ...
      } else if (ozonOrders > 0) {
        compositionNote = `<div class="ozon-fbs-containers-detail-empty">В Ozon указано заказов: ${esc(String(ozonOrders))}, список отправлений в ответе пуст.</div>`;
      } else {
        compositionNote = `<div class="ozon-fbs-containers-detail-empty">В ГМ пока нет заказов</div>`;
      }
    }
    const safeJs = JSON.stringify(key);
    const xlsLink = count
      ? `<a href="#" class="ozon-fbs-containers-composition-xls"
            title="Скачать состав в Excel"
            aria-label="Скачать состав ГМ ${esc(key)} в Excel"
            onclick='event.preventDefault(); downloadOzonFbsContainerComposition(${safeJs}); return false;'>↓</a>`
      : "";
  ...
  async function downloadOzonFbsContainerComposition(containerId) {
    ...
    const cached = containersState.detailsCache[cid];
    if (cached && !cached.error) {
      if (cached.warehouse_date) params.set("warehouse_date", String(cached.warehouse_date));
      if (cached.created_at) params.set("created_at", String(cached.created_at));
      if (cached.status) params.set("status", String(cached.status));
      if (cached.status_label) params.set("status_label", String(cached.status_label));
      if (cached.container_number != null) {
        params.set("container_number", String(cached.container_number || 0));
      }
    }
    _ozonFbsContainersSetInfo("Скачиваем состав ГМ…");
    try {
      const res = await fetch(
        `/api/ozon-fbs/supplies/${encodeURIComponent(sid)}/containers/${encodeURIComponent(cid)}/composition.xlsx?${params}`,
        { headers: jsonHeaders() }
      );
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
      ...
      if (m && m[1]) {
        try {
          ...
          fname = m[1].replace(/"/g, "").trim() || fname;
        }
      }
      ...
      _ozonFbsContainersSetInfo(`Скачан файл ${fname}`);
    } catch (e) {
      _ozonFbsContainersSetInfo(e.message || String(e), false);
    }
  }
  ...
  async function loadOzonFbsContainers({ keepInfo = false } = {}) {
    ...
    try {
      const params = _ozonFbsContainersListParams(sourceId);
      const res = await fetch(
  ...
  function _ozonFbsContainerApproveBuildBody({
    ...
    if (hasUnbound) {
      ...
      parts.push(
        ...
        + `Вы собираетесь подтвердить грузоместо. `
        + `После подтверждения изменить грузоместо будет нельзя. `
        + `Вы уверены?`
        + `</p>`
      );
    }
  ...
  function resolveShipmentsBarcode(data, _carriageId) {
    const wh = data?.warehouse_barcode;
    if (wh && typeof wh === "object") {
      const text = String(wh.barcode_text || "").trim();
      const labelB64 = String(wh.barcode_label_base64 || "").trim();
      const b64 = String(wh.barcode_image_base64 || "").trim();
      if (text || labelB64 || b64) return wh;
    }
  ...
  function shipmentsWarehouseCaption(data) {
    const name = String(data?.warehouse_name || "").trim();
    if (name) return `Штрихкод для склада ${name}`;
    const wid = Number(data?.warehouse_id || 0);
    if (wid > 0) return `Штрихкод для склада ${wid}`;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  const ozonFbsKizState = {
    ...
    /** While modal open: soft-poll marking/status so peer scans update fill counters. */
    modalStatusPollTimer: null,
    modalStatusFirstTimer: null,
    modalStatusBurstRetry: null,
    lastModalStatusPollAt: 0,
    /** Defer table rebuild when operator is typing in a KIZ cell. */
    peerDomRefreshPending: false,
  ...
  const ozonFbsPickState = {
    ...
    /** Re-run status once current refresh finishes (open+save race). */
    statusRefreshQueued: false,
    /** While modal open: soft-poll pick-verify/status for peer fill counters. */
    modalStatusPollTimer: null,
    modalStatusFirstTimer: null,
    modalStatusBurstRetry: null,
    lastModalStatusPollAt: 0,
  ...
  function _ozonFbsSyncRowMenuContent(menu) {
    if (!menu) return;
    const mode = String(menu.dataset.mode || "").trim();
    const pn = String(menu.dataset.posting || "").trim();
    if (mode !== "kiz" && mode !== "pick") return;
    if (!pn) return;
    const rows = mode === "pick"
      ? (ozonFbsPickState?.rows || [])
      : (ozonFbsKizState?.rows || []);
    const row = rows.find((r) => String(r?.posting_number || "").trim() === pn);
    if (!row) return;
    menu.innerHTML = _ozonFbsModalRowMenuItemsHtml(row, mode);
  }
  ...
  function _ozonFbsModalRowActionsHtml(row, mode) {
    ...
    // Prefix with mode so supply-detail ⋮ (same posting) does not steal getElementById.
    const menuKey = _ozonFbsPostingMenuKey(`${mode || "kiz"}_${pn}`);
    const safePn = esc(pn);
    return `<div class="wb-fbs-row-menu-wrap" id="ozonFbsRowMenuWrap_${menuKey}">
      <button type="button" class="icon-btn secondary wb-fbs-row-menu-btn" title="Действия"
              onclick="toggleOzonFbsRowMenu(event, '${menuKey}')" aria-haspopup="menu">⋮</button>
      <div id="ozonFbsRowMenu_${menuKey}" class="wb-fbs-row-menu" data-posting="${safePn}" data-mode="${esc(mode)}" role="menu">
        ${_ozonFbsModalRowMenuItemsHtml(row, mode)}
      </div>
    </div>`;
  }
  ...
  /** First silent pull after open — don't wait a full poll interval. */
  const OZON_FBS_MODAL_STATUS_FIRST_MS = 3000;
  ...
  /** Recent wedge/COM activity — used to skip network poll during a burst. */
  function _ozonFbsPeerScanBurstBusy() {
  ...
  /**
   * Defer heavy table rebuild while a burst is active OR a row/GM cell is focused.
   * Resting focus on the top sticker/mark scan field must NOT block tbody refresh —
   * those inputs live outside the table.
   */
  function _ozonFbsPeerDomBusy() {
    if (_ozonFbsPeerScanBurstBusy()) return true;
    const el = document.activeElement;
    if (!_ozonFbsPeerIsScanInput(el)) return false;
    const id = String(el.id || "");
    if (
      id === "ozonFbsKizStickerScan"
      || id === "ozonFbsKizMarkScan"
  ...
  /** @deprecated alias — prefer burst (poll) vs dom (rebuild) explicitly. */
  function _ozonFbsPeerScanBusy() {
    return _ozonFbsPeerDomBusy();
  }

  function _ozonFbsPeerSavedAtMs(value) {
  ...
  /** Remote wins only when strictly newer; never let a stale status wipe a fresh local save. */
  function _ozonFbsPeerRemoteIsNewer(localSavedAt, remoteSavedAt) {
    const localMs = _ozonFbsPeerSavedAtMs(localSavedAt);
    const remoteMs = _ozonFbsPeerSavedAtMs(remoteSavedAt);
    if (remoteMs <= 0 && localMs > 0) return false;
  ...
  function _ozonFbsKizMergeRemoteIntoOpenModal(orders) {
    ...
    if (!Array.isArray(ozonFbsKizState.rows) || !ozonFbsKizState.rows.length) return 0;
    ...
    for (const row of ozonFbsKizState.rows) {
      ...
      if (upd.kiz_required === false) continue;
      const curCodes = _ozonFbsKizNormalizeCodesList(row.kiz_codes);
      // Unsaved local edits vs baseline — leave alone.
      if (!_ozonFbsKizBaselineEquals(pn, curCodes)) continue;
      const nextCodes = _ozonFbsKizNormalizeCodesList(upd.kiz_codes);
      const sameCodes = curCodes.length === nextCodes.length
        && curCodes.every((c, i) => c === nextCodes[i]);
      const nextSaved = String(upd.kiz_saved_at || "").trim();
      const curSaved = String(row.kiz_saved_at || "").trim();
      if (sameCodes) {
        // Prefer the newer conflict token when codes already match.
        if (_ozonFbsPeerRemoteIsNewer(curSaved, nextSaved)) row.kiz_saved_at = nextSaved;
        continue;
      }
      // Codes differ: adopt only a strictly newer remote snapshot.
      if (!_ozonFbsPeerRemoteIsNewer(curSaved, nextSaved)) continue;
      row.kiz_codes = nextCodes.length ? nextCodes.slice() : [""];
      if (upd.kiz_status) row.kiz_status = String(upd.kiz_status);
      if (nextSaved) row.kiz_saved_at = nextSaved;
      if (!ozonFbsKizState.baselineByPosting) ozonFbsKizState.baselineByPosting = {};
      ozonFbsKizState.baselineByPosting[pn] = nextCodes.slice();
      if (ozonFbsKizState.forceSaveByPosting) delete ozonFbsKizState.forceSaveByPosting[pn];
      touched += 1;
    }
    if (!touched) {
      if (ozonFbsKizState.peerDomRefreshPending && !_ozonFbsPeerDomBusy()) {
        ozonFbsKizState.peerDomRefreshPending = false;
        renderOzonFbsKizTable({ skipCollect: true });
  ...
  function _ozonFbsKizStopModalStatusPoll() {
    if (ozonFbsKizState.modalStatusPollTimer != null) {
      clearInterval(ozonFbsKizState.modalStatusPollTimer);
      ozonFbsKizState.modalStatusPollTimer = null;
    }
    if (ozonFbsKizState.modalStatusFirstTimer != null) {
      clearTimeout(ozonFbsKizState.modalStatusFirstTimer);
      ozonFbsKizState.modalStatusFirstTimer = null;
    }
    if (ozonFbsKizState.modalStatusBurstRetry != null) {
      clearTimeout(ozonFbsKizState.modalStatusBurstRetry);
      ozonFbsKizState.modalStatusBurstRetry = null;
    }
    ozonFbsKizState.peerDomRefreshPending = false;
  ...
  function _ozonFbsKizStartModalStatusPoll() {
    ...
    ozonFbsKizState.lastModalStatusPollAt = 0;
    const tick = () => {
      ...
      // Burst only — resting focus on sticker must NOT block peer counter sync.
      if (_ozonFbsPeerScanBurstBusy()) {
        if (ozonFbsKizState.modalStatusBurstRetry == null) {
          ozonFbsKizState.modalStatusBurstRetry = setTimeout(() => {
            ozonFbsKizState.modalStatusBurstRetry = null;
            tick();
          }, OZON_FBS_PEER_SCAN_BUSY_MS + 100);
        }
        return;
      }
      const now = Date.now();
      ...
      ozonFbsKizState.lastModalStatusPollAt = now;
      void refreshOzonFbsMarkingStatus(null, { silent: true });
    };
    ozonFbsKizState.modalStatusFirstTimer = setTimeout(() => {
      ozonFbsKizState.modalStatusFirstTimer = null;
      tick();
    }, OZON_FBS_MODAL_STATUS_FIRST_MS);
    ozonFbsKizState.modalStatusPollTimer = setInterval(tick, OZON_FBS_MODAL_STATUS_POLL_MS);
  }
  ...
  async function openOzonFbsKizModal() {
    ...
    try {
      ...
      _ozonFbsKizStartModalStatusPoll();
      void _ozonFbsScanComOnModalOpened();
  ...
  async function refreshOzonFbsMarkingStatus(event, opts) {
    ...
    try {
      ...
      // Multi-op: adopt peer KIZ fills into the open modal (scan counter line 1).
      _ozonFbsKizMergeRemoteIntoOpenModal(data.orders || []);
  ...
  function _ozonFbsPickMergeRemoteIntoOpenModal(orders) {
    ...
    for (const row of ozonFbsPickState.rows) {
      ...
      const curBarcode = String(row.pick_barcode || "").trim();
      if (!_ozonFbsPickBaselineEquals(pn, curVerified, curBarcode)) continue;
      const nextVerified = !!upd.pick_verified && !!String(upd.pick_barcode || "").trim();
      const nextBarcode = nextVerified ? String(upd.pick_barcode || "").trim() : "";
      const nextAt = String(upd.pick_verified_at || "").trim();
      const curAt = String(row.pick_verified_at || "").trim();
      if (curVerified === nextVerified && curBarcode === nextBarcode) {
        if (_ozonFbsPeerRemoteIsNewer(curAt, nextAt)) row.pick_verified_at = nextAt;
        continue;
      }
      if (!_ozonFbsPeerRemoteIsNewer(curAt, nextAt)) continue;
      row.pick_verified = nextVerified;
      row.pick_barcode = nextBarcode;
      // Keep clear/fill token for the next conflict / peer compare.
      row.pick_verified_at = nextAt;
      _ozonFbsPickSetBaseline(pn, nextVerified, nextBarcode);
      if (ozonFbsPickState.forceSaveByPosting) delete ozonFbsPickState.forceSaveByPosting[pn];
      touched += 1;
    }
    if (!touched) {
      if (ozonFbsPickState.peerDomRefreshPending && !_ozonFbsPeerDomBusy()) {
        ozonFbsPickState.peerDomRefreshPending = false;
        renderOzonFbsPickVerifyTable();
  ...
  function _ozonFbsPickStopModalStatusPoll() {
    if (ozonFbsPickState.modalStatusPollTimer != null) {
      clearInterval(ozonFbsPickState.modalStatusPollTimer);
      ozonFbsPickState.modalStatusPollTimer = null;
    }
    if (ozonFbsPickState.modalStatusFirstTimer != null) {
      clearTimeout(ozonFbsPickState.modalStatusFirstTimer);
      ozonFbsPickState.modalStatusFirstTimer = null;
    }
    if (ozonFbsPickState.modalStatusBurstRetry != null) {
      clearTimeout(ozonFbsPickState.modalStatusBurstRetry);
      ozonFbsPickState.modalStatusBurstRetry = null;
    }
    ozonFbsPickState.peerDomRefreshPending = false;
  ...
  function _ozonFbsPickStartModalStatusPoll() {
    ...
    _ozonFbsPeerBindScanWatch();
    ozonFbsPickState.lastModalStatusPollAt = 0;
    const tick = () => {
      if (!_ozonFbsPickModalIsOpen()) {
        _ozonFbsPickStopModalStatusPoll();
        return;
      ...
      if (_ozonFbsPeerScanBurstBusy()) {
        if (ozonFbsPickState.modalStatusBurstRetry == null) {
          ozonFbsPickState.modalStatusBurstRetry = setTimeout(() => {
            ozonFbsPickState.modalStatusBurstRetry = null;
            tick();
          }, OZON_FBS_PEER_SCAN_BUSY_MS + 100);
        }
      ...
      ozonFbsPickState.lastModalStatusPollAt = now;
      void refreshOzonFbsPickVerifyStatus(null, { silent: true });
    };
    ozonFbsPickState.modalStatusFirstTimer = setTimeout(() => {
      ozonFbsPickState.modalStatusFirstTimer = null;
      tick();
    }, OZON_FBS_MODAL_STATUS_FIRST_MS);
    ozonFbsPickState.modalStatusPollTimer = setInterval(tick, OZON_FBS_MODAL_STATUS_POLL_MS);
  }
  ...
  async function _ozonFbsPickFlushLocalAutosave(postingNumber, seq, clear) {
    ...
    try {
      ...
      // Always adopt token (incl. clear) for peer/conflict compares.
      if (result.pick_verified_at != null) {
        row.pick_verified_at = String(result.pick_verified_at || "");
      }
  ...
  async function refreshOzonFbsPickVerifyStatus(event, opts) {
    ...
    try {
      ...
      if (Array.isArray(data.orders)) {
        ...
        // Multi-op: adopt peer ШК checks into the open pick modal (scan counter).
        _ozonFbsPickMergeRemoteIntoOpenModal(data.orders);
      }
      if (st === "error") {
        const tip = typeof _ozonFbsContainerErrorsTooltip === "function"
          ? _ozonFbsContainerErrorsTooltip(data.container_errors || [])
  ...
  async function openOzonFbsPickVerifyModal() {
    ...
    try {
      ...
      _ozonFbsPickStartModalStatusPoll();
      void _ozonFbsScanComOnModalOpened();
      if (loadOk && scan) setTimeout(() => {
        if (stillThisLoad()) scan.focus();
      }, 50);
  ...
  function _ozonFbsAllCancellationsSetInfo(text, kind) {
    const el = document.getElementById("ozonFbsAllCancellationsInfo");
    if (!el) return;
    const msg = String(text || "").trim();
    el.hidden = !msg;
    el.textContent = msg;
  ...
  function _ozonFbsAllCancellationsFilteredSupplies() {
    const q = String(allCancellationsState.search || "").trim().toLowerCase();
    const list = Array.isArray(allCancellationsState.supplies)
      ? allCancellationsState.supplies
      : [];
    if (!q) return list.slice();
    return list.filter((s) => {
      const sid = String(s?.supply_id || "").toLowerCase();
      const name = String(s?.name || "").toLowerCase();
      const wh = String(s?.warehouse_label || "").toLowerCase();
      if (sid.includes(q) || name.includes(q) || wh.includes(q)) return true;
      const orders = Array.isArray(s?.cancelled_orders) ? s.cancelled_orders : [];
      return orders.some((o) => {
        const pn = String(o?.posting_number || "").toLowerCase();
        const offer = String(o?.offer_id || "").toLowerCase();
        const pname = String(o?.product_name || "").toLowerCase();
        return pn.includes(q) || offer.includes(q) || pname.includes(q);
      });
    });
  }

  function _ozonFbsAllCancellationsOrdersHtml(orders) {
    const rows = Array.isArray(orders) ? orders : [];
    if (!rows.length) {
      return `<div class="ozon-fbs-all-cancels-empty">В этой поставке отменённых заказов нет</div>`;
    }
        ...
        <td>
          <div class="wb-fbs-product">
            ...
            <div class="wb-fbs-product-text">
              <div class="wb-fbs-product-name" title="${esc(r.product_name || r.offer_id || "")}">${esc(r.product_name || r.offer_id || "—")}</div>
              <div class="wb-fbs-product-sub">${esc(offerArt || "—")}</div>
              ${typeof productCompositionHtml === "function" ? productCompositionHtml(r) : ""}
              ${barcodeHtml}
              ${cancelBadgeHtml(r)}
            </div>
          </div>
    ...
    return `<div class="wb-fbs-kiz-table-wrap">
      <table class="wb-fbs-kiz-table">
        <thead>
          <tr>
            <th class="wb-fbs-kiz-col-order">Отправление</th>
            <th class="wb-fbs-kiz-col-product">Товар</th>
          </tr>
        </thead>
        <tbody>${body}</tbody>
      </table>
  ...
  function renderOzonFbsAllCancellationsList() {
    const root = document.getElementById("ozonFbsAllCancellationsList");
    const summary = document.getElementById("ozonFbsAllCancellationsSummary");
    if (!root) return;
    if (allCancellationsState.loading && !allCancellationsState.supplies.length) {
      root.innerHTML = `<div class="ozon-fbs-all-cancels-empty">Загрузка…</div>`;
      if (summary) summary.textContent = "";
      return;
    }
    if (allCancellationsState.lastError && !allCancellationsState.supplies.length) {
      root.innerHTML = `<div class="ozon-fbs-all-cancels-empty">${esc(allCancellationsState.lastError)}</div>`;
      if (summary) summary.textContent = "";
      return;
    }
    const filtered = _ozonFbsAllCancellationsFilteredSupplies();
    const cancelledShown = filtered.reduce(
      (n, s) => n + (Array.isArray(s.cancelled_orders) ? s.cancelled_orders.length : 0),
      0
    );
    if (summary) {
      summary.textContent = filtered.length
        ? `Поставок: ${filtered.length} · отмен: ${cancelledShown}`
        : "";
    }
    if (!filtered.length) {
      const emptyMsg = String(allCancellationsState.search || "").trim()
        ? "Ничего не найдено"
        : "На вкладке «Доставляются» поставок нет";
      root.innerHTML = `<div class="ozon-fbs-all-cancels-empty">${esc(emptyMsg)}</div>`;
      return;
    }
    root.innerHTML = filtered.map((s) => {
      const sid = String(s.supply_id || "").trim();
      const open = allCancellationsState.expanded.has(sid);
      const count = Number(s.cancelled_count || 0);
      const countCls = count > 0
        ? "ozon-fbs-all-cancels-supply-count"
        : "ozon-fbs-all-cancels-supply-count is-zero";
      const countLabel = count > 0 ? `${count} отм.` : "0 отм.";
      const meta = [
        s.warehouse_label ? String(s.warehouse_label) : "",
        `в доставке: ${Number(s.order_count || 0)}`,
      ].filter(Boolean).join(" · ");
      return `<div class="ozon-fbs-all-cancels-supply${open ? " is-open" : ""}" data-supply="${esc(sid)}" role="listitem">
        <button type="button" class="ozon-fbs-all-cancels-supply-head"
          ...
          <span class="ozon-fbs-all-cancels-supply-main">
            ...
            <span class="ozon-fbs-all-cancels-supply-meta">${esc(meta)}</span>
          </span>
          <span class="${countCls}">${esc(countLabel)}</span>
        </button>
        <div class="ozon-fbs-all-cancels-supply-body" ${open ? "" : "hidden"}>
          ${_ozonFbsAllCancellationsOrdersHtml(s.cancelled_orders)}
        </div>
      </div>`;
    }).join("");
  }

  function toggleOzonFbsAllCancellationsSupply(supplyId) {
    const sid = String(supplyId || "").trim();
    if (!sid) return;
    if (allCancellationsState.expanded.has(sid)) {
      allCancellationsState.expanded.delete(sid);
    } else {
      allCancellationsState.expanded.add(sid);
    }
    renderOzonFbsAllCancellationsList();
  }

  function onOzonFbsAllCancellationsSearchInput() {
    const input = document.getElementById("ozonFbsAllCancellationsSearch");
    allCancellationsState.search = String(input?.value || "");
    renderOzonFbsAllCancellationsList();
  }

  async function loadOzonFbsAllCancellations() {
    const sourceId = state.sourceId;
    if (!sourceId || allCancellationsState.loading) return;
    allCancellationsState.loading = true;
    allCancellationsState.lastError = "";
    _ozonFbsAllCancellationsSetInfo("");
    renderOzonFbsAllCancellationsList();
    try {
      const res = await fetch(
        `/api/ozon-fbs/cancellations/delivering?source_id=${encodeURIComponent(sourceId)}`
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data.ok === false) {
        throw new Error(detailText(data.detail) || `Ошибка ${res.status}`);
      }
      allCancellationsState.supplies = Array.isArray(data.supplies) ? data.supplies : [];
      allCancellationsState.lastError = "";
      renderOzonFbsAllCancellationsList();
    } catch (e) {
      allCancellationsState.lastError = String(e.message || e);
      _ozonFbsAllCancellationsSetInfo(allCancellationsState.lastError, "error");
      renderOzonFbsAllCancellationsList();
    } finally {
      allCancellationsState.loading = false;
      renderOzonFbsAllCancellationsList();
    }
  }

  function openOzonFbsAllCancellationsModal() {
    if (typeof isTenantOwner === "function" && !isTenantOwner()) {
      alert("Все отмены доступны только главному пользователю");
      return;
    }
    if (!state.sourceId) {
      alert("Выберите источник Ozon");
      return;
    }
    allCancellationsState.search = "";
    allCancellationsState.expanded = new Set();
    allCancellationsState.supplies = [];
    allCancellationsState.lastError = "";
    const search = document.getElementById("ozonFbsAllCancellationsSearch");
    if (search) search.value = "";
    if (typeof setModalVisibility === "function") {
      setModalVisibility("ozonFbsAllCancellationsModal", true);
    } else {
      document.getElementById("ozonFbsAllCancellationsModal")?.classList.remove("hidden");
    }
    loadOzonFbsAllCancellations().catch(() => {});
  }

  function closeOzonFbsAllCancellationsModal() {
    allCancellationsState.loading = false;
    if (typeof setModalVisibility === "function") {
      setModalVisibility("ozonFbsAllCancellationsModal", false);
    } else {
      document.getElementById("ozonFbsAllCancellationsModal")?.classList.add("hidden");
    }
  }
  ...
  window.downloadOzonFbsContainerComposition = downloadOzonFbsContainerComposition;
  ...
  window._ozonFbsSilentRefreshPostingStatus = (pn) => refreshOzonFbsModalPostingStatus(pn, { silent: true });
  window._ozonFbsApplyCancelledQuiet = _ozonFbsApplyCancelledQuiet;
