...
function renderSupplyLegalEntitiesTbody() {
  ...
  if (!_supplyLegalEntitiesCache.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty-cell">Юридические лица не добавлены</td></tr>';
    return;
  }
  _supplyLegalEntitiesCache.forEach((e, i) => {
    const tr = document.createElement("tr");
    tr.dataset.id = e.id;
    tr.innerHTML = `<td>${i+1}</td><td class="editable-cell">${esc(e.short_name||"")}</td><td class="editable-cell">${esc(e.full_name||"")}</td><td class="editable-cell">${esc(e.requisites||"")}</td><td class="editable-cell">${esc(e.signatories||"")}</td><td class="editable-cell">${esc(e.in_person||"")}</td>
      <td>
        <div class="row" style="gap:4px;flex-wrap:nowrap">
...
async function saveEditLegalEntity(id) {
  ...
  if (!short) return;
  await fetch(`/api/supply-legal-entities/${id}`, { method: "PATCH", headers: jsonHeaders(), body: JSON.stringify({ short_name: short, full_name: full, requisites: req, signatories: sig, in_person: inp }) }).catch(() => null);
...
function toggleAddLegalEntityForm(show) {
  ...
  form.classList.toggle("hidden", !show); form.style.display = show ? "" : "none";
  if (!show) {
    ["newLegalShortName","newLegalFullName","newLegalRequisites","newLegalSignatories","newLegalInPerson"].forEach((id) => { const el = document.getElementById(id); if(el) el.value=""; });
  }
}
...
async function saveSupplyLegalEntity() {
  ...
  const res = await fetch("/api/supply-legal-entities", { method: "POST", headers: jsonHeaders(), body: JSON.stringify({ short_name: short, full_name: full, requisites: req, signatories: sig, in_person: inp }) }).catch(() => null);
  if (!res || !res.ok) { const e = await res?.json().catch(()=>({})) || {}; if (info) { info.textContent = e.detail||"Ошибка"; info.style.color = "#b91c1c"; } return; }
  if (info) { info.textContent = "Сохранено"; info.style.color = "#16a34a"; }
